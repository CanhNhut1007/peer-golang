...
export GOPATH=/go/src/github.com/hyperledger/fabric/core/chaincode/shim:$GOPATH
#export CC_CONSTRUCTOR='{"Args":["init","ACFT","1000", "A Cloud Fan Token!!!","john"]}'
# export CC_NAME="nodecc"
export CC_NAME="medicalrecord"
export CC_PATH="medicalrecord"
...
function cc_instantiate {
    ...
    if [ $? == "0" ]; then
        ...
        peer chaincode invoke  -C $CC_CHANNEL_ID -n $CC_NAME -c '{"Args":["init"]}' \
...
case $OPERATION in
    ...
    "instantiate")
        ...
        ;;
    "query")
            peer chaincode query -C $CC_CHANNEL_ID -n $CC_NAME  -c '{"Args": ["queryAllRecords"]}'
            echo " "
            peer chaincode query -C $CC_CHANNEL_ID -n $CC_NAME  -c '{"Args": ["getHistoryForRecord","000000-0001"]}'
        ;;
    
    "invoke")
            peer chaincode invoke -C $CC_CHANNEL_ID -n $CC_NAME  -c '{"Args": ["createMedicalRecord","Nhut","Thach","Fermale","Person"]}'

            peer chaincode invoke -C $CC_CHANNEL_ID -n $CC_NAME  -c  '{"Args": ["createMedicalRecord", "Anh","Nguyen","Male","Person"]}'

            peer chaincode invoke -C $CC_CHANNEL_ID -n $CC_NAME  -c  '{"Args": ["createMedicalRecord", "Quyen","Phan","Male","Person"]}'

	    peer chaincode invoke -C $CC_CHANNEL_ID -n $CC_NAME -c '{"Args": ["deleteMedicalRecord","000000-0001"]}'
        ;;
